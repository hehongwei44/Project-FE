;(function($) {
    ...
    $.fn.datePicker = function(options) {
        ...
        function newDatepickerHTML() {
            ...
            /*构造月份的下拉列表*/
            var monthlist = '<ul name="month">';
            /*ps:for in语句也可以用来迭代数组*/
            for(var i in months) {
                monthlist += '<li value="' + i + '">' + months[i] + '</li>';
            }
            monthlist += '</ul>';
            /*同上,构造年份的下拉列表*/
            var yearlist = '<ul name="year">';
            for(var i in years) {
                yearlist += '<li>' + years[i] + '</li>';
            }
            yearlist += '</ul>';
            /*动态添加dom*/
            $("thead", table).append('<tr class="controls"><th colspan="7"><span class="prevMonth"></span><span class="year_wrap"></span>年<span class="month_wrap">10</span>月<span class="nextMonth"></span><span class="today"></span></th></tr>');
            $("thead", table).append('<tr class="days"><th>日</th><th>一</th><th>二</th><th>三</th><th>四</th><th>五</th><th>六</th></tr>');
            $("tfoot", table).append('<tr><td colspan="7"></td></tr>');
            $("thead", table).append(yearlist);
            $("thead", table).append(monthlist);
            /*表的主体结构为5行7列*/
            for(var i = 0; i < 6; i++) {
        ...
        function loadMonth(e, el, datepicker, chosendate) {
            //mo表示选中月的索引
            var mo = $(".month_wrap", datepicker).text();
            //yr表示选中年的索引
            var yr = $(".year_wrap", datepicker).text();
            //年份类表的长度
            var yrs = $("ul[name=year] li", datepicker).length;
            /*单击向前或向后时年份与月份的变化*/
            if(e && $(e.target).hasClass('prevMonth')) {
                if(0 == mo && yr) {
                    yr -= 1;
                    mo = 12;
                    $(".month_wrap", datepicker).text(12);
                    $(".year_wrap", datepicker).text(yr);
                } else {
                    mo -= 1;
                    $(".month_wrap", datepicker).text(mo);
            ...
            for(var i = 0; i < numdays; i++) {
                ...
                if((yr || ((!startDate && !startMonth) || ((i + 1 >= startDate && mo == startMonth) || mo > startMonth))) && (yr + 1 < yrs || ((!endDate && !endMonth) || ((i + 1 <= endDate && mo == endMonth) || mo < endMonth)))) {
                    ...
                    if(text == chosendate.getDate() && m == chosendate.getMonth() && y == chosendate.getFullYear()) {
                        cell.empty().text(text).prepend("<sup class='chosen'>今</sup>");
                    }
            ...
            if($(this).is('input') && 'text' == $(this).attr('type')) {
                ...
                _this.bind("click focus", function(ev) {
                    ...
                    if(false == $.data($this.get(0), "datePicker").hasDatepicker) {
                        ...
                        $(".month_wrap", datepicker).text(chosendate.getMonth() + 1);
                        $(".year_wrap", datepicker).text(chosendate.getFullYear());
                        loadMonth(null, $this, datepicker, chosendate);
